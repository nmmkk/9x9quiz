...
* Manual browser verification: pending desktop/mobile confirmation in-session (CLI environment has no interactive browser UI)
...
| QA-08 | Manual online -> offline transition on desktop browser | Pending | Requires interactive browser session (not available in CLI runtime) |
...
* Pending external/manual:
  * Desktop browser manual online/offline transition scenario.
...
* Manual desktop/mobile verification is still required before final M5 closure.
