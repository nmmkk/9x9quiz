...
1. Rerun manual install/page-load checks on mobile with the latest service-worker fallback fix, then update `docs/qa/reports/2026-02-18-m05-qa-rerun-2.md` with pass/fail evidence.
2. After mobile verification passes, set `M5-04` to `Completed`, mark milestone `M5` as `Completed`, and start `M6-01`.
