...
| M9 | Partial | `M9-01` through `M9-03` completed in `docs/roadmap/implementation/m09-release-provenance-visibility.md`; `src/shared/buildInfo.ts` now formats build provenance with explicit fallback values, and `src/features/title/ui/TitleScreen.tsx` plus `src/styles/app.css` render a low-emphasis title-screen footer that links to the exact commit when URL metadata exists; `npm run test` (pass, 16 files / 59 tests); `npm run build` (pass); `npx vite --host 127.0.0.1 --port 4173 --strictPort` (booted) | The app now surfaces `version + short SHA` directly on the title screen with a safe non-link fallback for local or incomplete metadata. Remaining work is to verify the published app footer and record release evidence in `M9-04`. |
