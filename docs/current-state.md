...
| M8 | Partial | `M8-01` completed in `docs/roadmap/implementation/m08-post-release-learning-flow-polish.md`; `npm run test` (pass, 15 files / 57 tests); `npm run build` (pass, injectManifest precache 8 entries / 248.89 KiB); `npm run dev -- --host 127.0.0.1 --port 4301 --strictPort` (booted) | Post-release learning flow polish is in progress: session progress gauge/icon feedback is shipped, with direct-jump and reset-flow tickets (`M8-02`, `M8-03`) queued before `M8-04` QA/doc sync. |
