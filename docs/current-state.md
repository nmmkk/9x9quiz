...
| M4 | Partial | `M4-01` through `M4-05` completed in `docs/roadmap/implementation/m04-adaptive-practice-depth.md`; `npm run test` (pass, 11 files / 44 tests); `npm run build` (pass) | Adaptive-practice regression coverage now includes practice-scope filtering, mini-review trigger/session-length behavior, and mastery storage read/write normalization to guard M4 behavior. |
