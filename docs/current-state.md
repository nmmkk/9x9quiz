...
| M3 | Partial | `M3-01` completed in `docs/roadmap/implementation/m03-repeat-engagement-loop.md`; `npm run test` (pass, 5 files / 19 tests); `npm run build` (pass); `npm run dev -- --host 127.0.0.1 --port 4191 --strictPort` (booted) | M3-01 is done (title quick start via last played mode). Remaining scope: M3-02 through M3-05. |
