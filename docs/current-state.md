...
| M7 | Partial | `M7-01` and `M7-02` completed; `M7-03` partially implemented in `docs/roadmap/implementation/m07-web-release-cicd.md`; `.github/workflows/deploy.yml` added with `workflow_dispatch` and `push` triggers; `vite.config.ts` build base-path set to `/9x9quiz/`; `README.md` canonical URL updated; `docs/qa/reports/2026-02-20-m07-03-deploy-baseline.md`; `docs/qa/reports/2026-02-20-m07-03-deploy-preflight.md`; `npm run test` (pass); `npm run build` (pass); `gh api repos/nmmkk/9x9quiz/branches/main/protection` (required check `CI / test-build` configured, `strict: true`) | CI gate and Pages deploy pipeline code are in place, including manual preflight trigger support. `M7-03` closeout still needs one workflow run, one deploy run from `main`, and published URL smoke verification evidence. |
