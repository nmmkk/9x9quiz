...
| M4 | Partial | `M4-01` through `M4-03` completed in `docs/roadmap/implementation/m04-adaptive-practice-depth.md`; `npm run test` (pass, 9 files / 34 tests); `npm run build` (pass); `npm run dev -- --host 127.0.0.1 --port 4215 --strictPort` (booted) | Adaptive practice now includes missed-fact mini-review from Result: the button appears only when misses exist, launches a 3-question-or-fewer focused review set, and returns cleanly to Title flow after completion. |
