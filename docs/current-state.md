...
| M4 | Partial | `M4-01` through `M4-02` completed in `docs/roadmap/implementation/m04-adaptive-practice-depth.md`; `npm run test` (pass, 9 files / 34 tests); `npm run build` (pass); `npm run dev -- --host 127.0.0.1 --port 4214 --strictPort` (booted) | Practice scope persistence and mode-selection table-range controls are complete; learners can choose simple range presets before starting while existing high-score and mode flow remain stable. |
