...
| M7 | Partial | `M7-01` completed; `M7-02` partially implemented in `docs/roadmap/implementation/m07-web-release-cicd.md`; `.github/workflows/ci.yml` added; `docs/reference/web-release-strategy.md` and `docs/release/checklist.md` updated; `npm run test` (pass); `npm run build` (pass); `gh api repos/nmmkk/9x9quiz/branches/main/protection` (403: plan/visibility limitation) | CI workflow and reusable runbook are ready. Automated required-check enforcement is currently blocked by repository plan/visibility, so merge gating stays manual until that constraint is removed. |
