...
| M9 | Partial | `M9-01` and `M9-02` completed in `docs/roadmap/implementation/m09-release-provenance-visibility.md`; `vite.config.ts`, `src/vite-env.d.ts`, `.github/workflows/ci.yml`, and `.github/workflows/deploy.yml` now inject exact-build version/SHA metadata plus canonical commit URLs for local and CI/deploy builds; `npm run test` (pass, 14 files / 55 tests); `npm run build` (pass) | The provenance contract and metadata plumbing are now in place, including deterministic local fallback values and exact-SHA commit links for build artifacts. Remaining work is to render the title-screen footer (`M9-03`) and verify the published app (`M9-04`). |
