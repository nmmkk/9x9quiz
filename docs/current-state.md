...
| M7 | Partial | `M7-01` and `M7-02` completed; `M7-03` partially implemented in `docs/roadmap/implementation/m07-web-release-cicd.md`; `.github/workflows/deploy.yml` now triggers on successful `CI` workflow runs for `main` and allows `workflow_dispatch` on `main` only; `vite.config.ts` build base-path set to `/9x9quiz/`; `README.md` canonical URL updated; `docs/qa/reports/2026-02-20-m07-03-deploy-baseline.md`; `docs/qa/reports/2026-02-20-m07-03-deploy-preflight.md`; `npm run test` (pass); `npm run build` (pass); `gh api repos/nmmkk/9x9quiz/branches/main/protection` (required check `CI / test-build` configured, `strict: true`) | Deploy policy now prevents non-main manual publishes and gates automatic deploy on successful CI. `M7-03` closeout still needs one manual run on `main`, one CI-success deploy run on `main`, and published URL smoke verification evidence. |
