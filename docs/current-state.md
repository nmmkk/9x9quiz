...
| M8 | Partial | `M8-01` through `M8-03` completed in `docs/roadmap/implementation/m08-post-release-learning-flow-polish.md`; `npm run test` (pass, 16 files / 63 tests); `npm run build` (pass, injectManifest precache 8 entries / 250.38 KiB); `npm run dev -- --host 127.0.0.1 --port 4303 --strictPort` (booted) | Post-release learning flow polish is in progress: progress feedback, mastery-card direct jump, and protected reset flow are shipped; `M8-04` QA/doc sync remains to close the milestone. |
