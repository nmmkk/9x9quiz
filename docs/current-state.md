...
| M5 | Partial | `M5-01` and `M5-02` completed in `docs/roadmap/implementation/m05-offline-ready-pwa-foundation.md`; `npm run test` (pass, 13 files / 51 tests); `npm run build` (pass, injectManifest precache 4 entries); `npm run dev -- --host 127.0.0.1 --port 4236 --strictPort` (booted) | Offline app-shell caching plus deterministic update-notice flow are in place; `M5-03` and `M5-04` remain planned. |
