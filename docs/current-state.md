...
| M7 | Partial | `M7-01` completed; `M7-02` partially implemented in `docs/roadmap/implementation/m07-web-release-cicd.md`; `.github/workflows/ci.yml` added; `docs/reference/web-release-strategy.md` updated with CI policy and reusable GitHub Pages runbook; `npm run test` (pass); `npm run build` (pass) | PR/push CI pipeline and required status-check policy are documented and codified. Remaining M7-02 closeout is external verification: first PR CI pass and branch protection dry-run in GitHub settings. |
