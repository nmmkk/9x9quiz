...
| M4 | Partial | `M4-01` through `M4-04` completed in `docs/roadmap/implementation/m04-adaptive-practice-depth.md`; `npm run test` (pass, 9 files / 34 tests); `npm run build` (pass) | Adaptive practice now includes per-table mastery visibility on Title/Mode screens, with local-only answered/correct tracking that updates after each completed session while preserving existing flow. |
