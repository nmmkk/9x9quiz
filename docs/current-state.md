...
| M9 | Partial | `M9-01` through `M9-03` completed in `docs/roadmap/implementation/m09-release-provenance-visibility.md`; `M9-04` evidence is recorded in `docs/qa/reports/2026-03-23-m09-qa.md`; `npm run test` (pass, 16 files / 59 tests); `npm run build` (pass); `npx vite --host 127.0.0.1 --port 4173 --strictPort` (booted); published URL `https://nmmkk.github.io/9x9quiz/` responds; latest successful Pages deploy run `23472893920` targets `main` SHA `a1ffba2`, which does not yet match M9 branch head `2208ff4` | Release provenance implementation is complete locally, and the remaining closeout work is post-merge published verification. M9 stays partial until the milestone branch is merged, deployed, and the live footer is confirmed on desktop/mobile. |
