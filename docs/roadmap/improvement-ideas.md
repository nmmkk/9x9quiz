...
| B-02 | 段ごとの正解率カードから段別問題へ直行 | High | Medium-High | Completed in M8 (`M8-02`) | 習熟カードから練習開始までの遷移を短縮し、反復導線を強化する。 |
| B-03 | 成績リセット機能 | Medium | Medium | Completed in M8 (`M8-03`) | 端末共有時の再スタート要求に対応。誤操作防止のUIが前提。 |
